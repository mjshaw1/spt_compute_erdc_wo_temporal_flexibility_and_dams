...
def run_ecmwf_forecast_process(rapid_executable_location,  # path to RAPID executable
                               ...
                               # doesn't appear to ever be used... watersheds_with_dams_list=[], # a list of all watersheds where dam outflows are being forced
                               # doesn't appear to ever be used... stream_ids_with_dams_dict={}, # a dictionary with the watershed key and a value of a list of stream IDs where dams are located
                               # doesn't appear to ever be used... dam_outflows={} # a dictionary with the key as a stream ID and a value of a list of outflows
                               BS_opt_dam=False,
                               IS_dam_tot=0,
                               IS_dam_use=0,
                               dam_tot_id_file="",
    ...
    with CaptureStdOutToLog(log_file_path):
        ...
        print('ECMWF folders: ',ecmwf_forecast_location,
                                                     date_string,ecmwf_folders)
        # LOAD LOCK INFO FILE
        last_forecast_date = datetime.datetime.utcfromtimestamp(0)
        print('Last forecast date in ecmwf...: ',last_forecast_date)
        if os.path.exists(LOCK_INFO_FILE):
            with open(LOCK_INFO_FILE) as fp_lock_info:
                print('FP LOCK INFO?',fp_lock_info)
            ...
            if previous_lock_info['running']:
                ...
                print('Last forecast date before going into folders: ',last_forecast_date)
                run_ecmwf_folders = []
                for ecmwf_folder in ecmwf_folders:
                    # get date
                    forecast_date = get_datetime_from_forecast_folder(ecmwf_folder)
                    print('ECMWF folder...',ecmwf_folder)
                    print('Forecast date from ecmwf folder: ',forecast_date)
