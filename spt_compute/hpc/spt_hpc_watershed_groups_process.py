...
def spt_hpc_watershed_groups_process(main_log_directory,
    ...
    print('LOCK file: ',LOCK_INFO_FILE)
    print('Log files: ',log_file_path)
    with CaptureStdOutToLog(log_file_path):
        ecmwf_folders = sorted(get_ftp_forecast_list('Runoff.*.netcdf.tar*',
                                                     ...
                                                     ftp_passwd,
                                                     ftp_directory))
        print('ECMWF folders in spt_compute_hpc before doing much...',ecmwf_folders) 
        # LOAD LOCK INFO FILE
        last_forecast_date = datetime.datetime.utcfromtimestamp(0)
        print('Last forecast date in spt_compute_hpc before doing much...',last_forecast_date)
        if os.path.exists(LOCK_INFO_FILE):
            ...
            if previous_lock_info['running']:
                ...
                print('Last forecast date before running through ecmwf folders in :',lasts_forecast_date)
                run_ecmwf_folders = []
                for ecmwf_folder in ecmwf_folders:
                    # get date
                    forecast_date = get_datetime_from_forecast_folder(ecmwf_folder)
                    print('ECMWF folder in spt_compute_hpc...',ecmwf_folder)
                    print('Forecast date in spt_compute_hpc...',forecast_date)
